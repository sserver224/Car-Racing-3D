#Not working as well-using pygame._sdl2 for controller instead of XInput-python for macOS and linux controller support
#Problems:
#Not hot-swappable
#Controller disconnects from game after one race
import time
errorlist=[]
loadfailed=False
elm_missing=False
...
try:
    ...
    import pygame._sdl2.controller as DInput
except ImportError as e:
...
try:
    pg.init()
    DInput.init()
    controller=DInput.Controller(0)
except pg.error:
    loadfailed=True
def get_connected():
    return (controller.attached(), False, False, False)
def get_state(index):
    return 0
def get_thumb_values(state, deadzone):
    x=controller.get_axis(pg.CONTROLLER_AXIS_LEFTX)
    y=-1*controller.get_axis(pg.CONTROLLER_AXIS_LEFTY)
    rx=controller.get_axis(pg.CONTROLLER_AXIS_RIGHTX)
    ry=-1*controller.get_axis(pg.CONTROLLER_AXIS_RIGHTY)
    if x<0:
        x+=1
    if y>0:
        y-=1
    ...
    if abs(ry)<deadzone:
        ry=0
    return((x/32767, y/32767), (rx/32767, ry/32767))
def get_trigger_values(state):
    lt=min(controller.get_axis(pg.CONTROLLER_AXIS_TRIGGERLEFT)/32767, 1)
    rt=min(controller.get_axis(pg.CONTROLLER_AXIS_TRIGGERRIGHT)/32767, 1)
    return(lt, rt)
def set_vibration(index, left, right):
    if index==0:
        controller.rumble(left, right, 0)
def get_button_values(state):
    return{'BACK':controller.get_button(pg.CONTROLLER_BUTTON_BACK), 'START':controller.get_button(pg.CONTROLLER_BUTTON_START)}
try:
...
if len(errorlist)>0:
    ...
    if tkinter_available:
        messagebox.askokcancel('Warning', 'sdl2.controller failed to load. Press OK to load the game without controller support, or Cancel to quit.', icon='warning')
    else:
        print('sdl2.controller failed to load. Controller support will be disabled.')
...
Developed by sserver224\nmywebsite1324.neocities.org"
    ...
    if address!='0.0.0.0':
        th=Thread(target=sock.sendto, args=[bytes(str(DATA_OUT_FORMAT), 'utf-8'), (address, port)], daemon=True)
        th.start()
    ...
    while running:
        ...
        if get_connected()[0]:
            ...
            if get_button_values(get_state(0))['START']:
                ...
                walls = track.load_walls(car.x)
                rays = generate_rays(car.look_angle, fov, screen_width, res)
            steering=get_thumb_values(get_state(0), deadzone)[0][0]
        for event in pg.event.get():
            if event.type == pg.QUIT:
                exit_program()
...
if __name__ == "__main__":
    ...
    try:
        ...
        deadzone=QueryValueEx(OpenKey(OpenKey(OpenKey(HKEY_CURRENT_USER, 'Software', reserved=0, access=KEY_ALL_ACCESS), 'sserver', reserved=0, access=KEY_ALL_ACCESS), 'Car Racing 3D', reserved=0, access=KEY_ALL_ACCESS), 'SteeringDeadZone')[0]
